...
export default function SecretaryLayout({ children }) {
  ...
  // Filter menu items based on secretary permissions and adjust paths
  const menuItems = MENU_ITEMS.filter(item =>
    item.roles.includes('secretary') && hasPermission(user?.role, item.module, 'view')
  ).map(item => ({
    ...item,
    path: item.path.replace('/admin', '/secretary')
  }));
            ...
            {sidebarOpen ? (
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Secrétariat</p>
                </div>
              </div>
            ) : (
              <div className="w-10 h-10 bg-gradient-to-br from-green-500 to-teal-600 rounded-lg flex items-center justify-center mx-auto">
                <Activity className="w-6 h-6" />
              </div>
            )}
          </div>
          ...
          <div className="px-8 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">
                Bienvenue sur votre interface de secrétariat
              </p>
            </div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              {user?.email || 'secretaire@clinique.com'}
                            </p>
                          </div>
                        </div>
                      </div>
